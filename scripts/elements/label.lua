...
local Textbox = Element:new()

function Textbox:new(workspace)
    local tb = Element.new(Textbox)

    tb.core = rl.getTextbox()
...
Etiam vehicula, lorem mollis laoreet tristique, leo ex tristique risus, quis varius orci elit mollis est. Phasellus semper dignissim est, sed rutrum felis eleifend quis. Cras mattis et risus et volutpat. Fusce et scelerisque arcu. Duis fringilla placerat vestibulum. Maecenas pharetra nisl in ante pellentesque facilisis. Aliquam imperdiet mauris vitae molestie malesuada. Curabitur a lectus vel est dictum molestie non vel urna. Morbi tortor mi, congue eu enim nec, aliquet viverra justo. Vestibulum nec dui vitae leo eleifend rhoncus et nec lectus. Vivamus auctor felis quam, sed blandit velit finibus nec. In odio turpis, hendrerit vel sem in, faucibus aliquet nunc. Vivamus eget erat vitae dolor faucibus dapibus. Quisque suscipit finibus laoreet]]
    ...
    local resize = tb.resize
    tb.resize = function(self, ...)
        resize(self, ...)
...
local TextboxBuilder = {}

function TextboxBuilder:new(textbox, mousePos)
    local tbb = {}
    setmetatable(tbb, self)
    self.__index = self
    ...
    tbb.textOffsetX = 0
    tbb.textOffsetY = 0
    tbb.peakTextOffsetX = 0

    return tbb
end

function TextboxBuilder:EndOfLineCursorCheck()
...
function TextboxBuilder:InsertBuffer(lineBuffer, codepointBuffer)
...
function Textbox:prepTB(x, y, w, h)
    -- self.core:PrepareTexbox(self.text, x, y, w, h)
    self.lines = {}

    local CpC = rl.getCodepointCounter(self.text, self.fontName, self.fontSize, self.spacing)
    local TBB = TextboxBuilder:new(self, false) -- true if i have a cursor position
    ...
    while CpC:hasNext() do
        ...
        if codepoint == 10 then -- if newline
            TBB:InsertBuffer(lineBuffer, codepointBuffer)
            codepointBuffer = {}

            TBB:EndOfLineCursorCheck()

            TBB:InsertBuffer(lineBuffer, {{32, 0}}) -- insert space with no width at end for making it work well, iirc
            table.insert(self.lines, lineBuffer)
            lineBuffer = {}
            codepointBufferWidth = 0
            lineWidth = 0

            TBB.textOffsetY = TBB.textOffsetY + self.lineSpacing + self.fontSize

            TBB.peakTextOffsetX = math.max(TBB.peakTextOffsetX, TBB.textOffsetX)
            TBB.textOffsetX = 0
        else
            ...
            if codepoint == 32 or codepoint == 9 or self.wrapping == 0 or self.wrapping == 2 or TBB.curLineWidth + nextCharWidth + codepointBufferWidth > w then
    ...
    if TBB.setCursor then
        self.cursorPosition = #self.text -- end of text
    end

    if self.cursorVisualX == -1 then
        self.savedCursorVisualX = -1

        self.cursorVisualX = TBB.textOffsetX
        self.cursorVisualY = TBB.textOffsetY
    end
    TBB.peakTextOffsetX = math.max(TBB.peakTextOffsetX, TBB.textOffsetX)
    
    -- would then have scrolling stuff here - but i might want that to be
    -- made as a seperate element
end

function Textbox:draw()
    -- self.core:DrawCodepoints(self.es.x, self.es.y, self.es.w, self.es.h)
    ...
    for _, line in pairs(self.lines) do
        for _, char in pairs(line) do
            ...
            if char.codepoint ~= 32 and char.codepoint ~= 9 then
                rl.drawTextCodepoint(font, char.codepoint, rl.vec(self.es.x + textOffsetX, self.es.y + textOffsetY), self.fontSize, rl.color(0, 0, 255));
            end
